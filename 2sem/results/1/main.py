...
def semitone(input_path: str, output_path: str) -> None:
    ...
    new_img.save(output_path + ".bmp", bitmap_format="bmp")
    print(f"Wrote semitone in {output_path}.bmp")
...
def main():
    ...
    for input_path in glob(relative_path):
        curr_opath = os.path.join(output_path, os.path.splitext(os.path.basename(input_path))[0])
